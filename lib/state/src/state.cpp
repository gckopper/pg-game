...
void gm::add_text(const std::string text, const Coordinate pos, const float size, Font& font) {
    ...
    if (((font.offset/FONT_ATTRIB) + text.size()) > MAX_TEXT_SIZE) {
    ...
    for (uint64_t i = 0; i < text.size(); ++i) {
        ...
        font.vbo_data[font.offset + i * FONT_ATTRIB]     = pos.x + (i * width);
        font.vbo_data[font.offset + i * FONT_ATTRIB + 1] = pos.y;
        font.vbo_data[font.offset + i * FONT_ATTRIB + 2] = coord.x;
        font.vbo_data[font.offset + i * FONT_ATTRIB + 3] = coord.y;
        font.vbo_data[font.offset + i * FONT_ATTRIB + 4] = size;
    }

    glBindBuffer(GL_ARRAY_BUFFER, font.vbo);
    glBufferSubData(GL_ARRAY_BUFFER, font.offset * sizeof(GLfloat),
                    text.size() * FONT_ATTRIB * sizeof(GLfloat),
                    font.vbo_data.data() + font.offset);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    font.offset += text.size()*FONT_ATTRIB;
}
